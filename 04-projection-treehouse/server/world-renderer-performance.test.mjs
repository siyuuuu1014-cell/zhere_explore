...
test('desktop movement reuses terrain chunks and leaves ambient life on its slower loop', async () => {
  ...
  assert.match(prototypeSource, /const AMBIENT_UPDATE_INTERVAL_MS = 100/);
  ...
  assert.doesNotMatch(movementFrame, /updateAmbientLife/);
});
